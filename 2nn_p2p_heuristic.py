...
def add_nghbr_comm(nghbrs_idx_comm, client_idx, nghbr_idx):
    nghbrs_idx_comm[client_idx].append(nghbr_idx)

    # Max 10 in this list.
    if len(nghbrs_idx_comm[client_idx]) > 10:
        nghbrs_idx_comm[client_idx].pop(0)

    return nghbrs_idx_comm    
...
def differences_neighbors(client_idx, nghbrs_idx_comm):
    client_nghbrs = set(nghbrs_idx_comm[client_idx])
    diffs = []
    for i, nghbr_comm in enumerate(nghbrs_idx_comm):
        diffs.append((i, len(set(nghbr_comm) - client_nghbrs)))
...
def update_models(clients, graph, nghbrs_idx_comm, device, C):
    """
        Update client models by aggregating and averaging all clients models with neighbors.
    """
    new_clients = copy.deepcopy(clients)
    # Calc neighbors that have communicated with different clients!
    neighbors_diffs = []
    for client_idx, client in enumerate(new_clients):
        neighbors_diffs.append(differences_neighbors(client_idx, nghbrs_idx_comm))
    ...
    for client_idx, client in enumerate(new_clients):
        ...
        for neighbor_idx in neighbor_idxs:
            # Add neighbor
            nghbrs_idx_comm = add_nghbr_comm(nghbrs_idx_comm, client_idx, neighbor_idx)
...
def decentralized_learning():
    ...
    for round in range(1, communication_rounds+1):
        ...
        clients = update_models(clients, graph, nghbrs_idx_comm, device, C)
